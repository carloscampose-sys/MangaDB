...
/**
 * Construye query string sin codificar los corchetes (requerido por MangaDex)
 */
...
export async function searchManga(query, limit = 20) {
  try {
    ...
    const url = `${BASE_URL}/manga?${buildQueryString(params)}`;
...
export async function getMangaDetails(mangaId) {
  try {
    ...
    const response = await fetch(`${BASE_URL}/manga/${mangaId}?${buildQueryString(params)}`);
...
export async function getMangaChapters(mangaId, offset = 0, limit = 100) {
  try {
    ...
    const response = await fetch(`${BASE_URL}/chapter?${buildQueryString(params)}`);
