 ...
 */
export async function getMangaChapters(mangaId, offset = 0, limit = 100, language = 'es') {
  try {
    // MangaDex tiene un límite máximo de 100 por request
    const maxLimit = Math.min(limit, 100);
    // Buscar capítulos en el idioma especificado, o todos si es 'all'
    const langParam = language === 'all' ? '' : `&translatedLanguage[]=${language}`;
    const url = `${BASE_URL}/chapter?manga=${mangaId}${langParam}&order[chapter]=asc&offset=${offset}&limit=${maxLimit}`;
