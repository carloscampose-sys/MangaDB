...
function parseChaptersFromProtobuf(buffer) {
  ...
  const seenChapters = new Set(); // Para evitar duplicados por número de capítulo
  ...
  while ((blockMatch = chapterBlockPattern.exec(data)) !== null) {
    ...
    const searchArea = data.substring(Math.max(0, blockPos - 150), blockPos + 300);

    // Buscar #001, #002, etc
    const numMatch = searchArea.match(/#(\d{2,4})/);
    // Buscar "Capítulo X: Título"
    const titleMatch = searchArea.match(/Cap[^\x00]{0,10}tulo\s*(\d+)[:\s]*([^\x00\x08\x10\x12\x1a\x22]{0,50})/i);

    let chapterNum;
    let chapterTitle;

    if (numMatch) {
      chapterNum = parseInt(numMatch[1]).toString();
    } else if (titleMatch) {
      chapterNum = titleMatch[1];
    } else {
      chapterNum = (chapters.length + 1).toString();
    }

    // Evitar duplicados por número de capítulo (quedarse con el primero)
    if (seenChapters.has(chapterNum)) continue;

    if (titleMatch && titleMatch[2] && titleMatch[2].trim().length > 0) {
      chapterTitle = `Capítulo ${chapterNum}: ${cleanString(titleMatch[2])}`;
    } else {
